...
route_layer parse_route(list *options, size_params params, network *net)
{
    ...
    int *out_chnel = calloc(n, sizeof(int));
    for(i = 0; i < n; ++i){
        int index = atoi(l);
        l = strchr(l, ',')+1;
        if(index < 0) index = params.index + index;
        ...
        out_chnel[i] = net->layers[index].out_c;
    }
    int w = params.w;
    int h = params.h;
    int batch = params.batch;

    route_layer layer = make_route_layer(batch, n, layers, sizes, out_chnel, w, h);
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        if(l.type == CONNECTED){
		 ...
		 if(l.type == ROUTE){
            load_route_weights(l, fp, transpose);
        }
        if(l.type == BATCHNORM){
